...
- **`Core.lua`** — debug ring buffer (`Dbg`, `ShowDebugWindow`), name normalization (`NormalizeName`, `FullName`), display helpers (`GetDungeonName`, `SpecName`, `ClassFromSpec`, `GetClassColor`, `KeyLevelColor`, `SetRoleIcon`, `FormatDuration`, `EstimateMinTimedScore`), Raider.IO URL builder + StaticPopup. Owns the SeanKeys container frame and the debug window, including the footer "Listen for ESC to close windows" checkbox that backs `ns.db.options.listenForEsc` (read by `GetContainer` at build time, so changes need `/reload`). All exposed on `ns`.
...
- `consumableProfiles = { { name, lists = { [category] = { { itemID, quality, target }, ... } } }, ... }` — consumable shopping lists organized into named profiles. `category` ∈ `food | flask | potion | weaponEnchant | other`. Each entry's identity is `(itemID, quality)` — adds always insert at `quality=1`, and the picker greys out items that already have a quality-1 row in this profile's list. `quality` is capped at 2 (Midnight crafting tier).
- `activeConsumableProfile` — integer index into `consumableProfiles`, defaults to 1. The AH tab's profile dropdown switches this.
- `MigrateSpecToProfiles` in `ConsumableShopping.lua` rolls the previous spec-keyed schema (`consumables[specID][category]`) forward: each non-empty spec entry becomes its own profile named after the spec. `MigrateLegacyKeys` separately folds older category names (`combatPotion`, `healthPotion` → `potion`; `augmentRune` → `other`) and backfills missing `quality = 1`.
...
4. **`CURRENT_SEASON_CONSUMABLES`** (in `ConsumableShopping.lua`) — `[category] = { { qualities|itemID, name }, ... }`. The catalog the consumables-settings picker offers under each category. Crafted consumables with multiple quality tiers use `qualities = { q1ID, q2ID }` (Midnight caps at T2 — no T3); single-quality items use `itemID = N`. A normalization pass after the table literal collapses both shapes to a uniform `{ itemID = qualities[1], qualities = {...} }`. Order in each list controls the picker order. The `name` is just a fallback for the picker UI; we resolve real names via `C_Item.GetItemInfo` once items are cached. The dropdown on each row rewrites `entry.itemID` to the chosen quality's variant via `ResolveQualityItemID`, so AH searches, bag counts, and icons all reflect the active quality.
