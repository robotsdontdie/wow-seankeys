...
local function Render()
	...
	for _, item in ipairs(flat) do
		...
		-- Small clear-X button. UIPanelCloseButton's textures don't scale below
		-- ~32px so a shrunk version renders effectively blank; build our own.
		local removeBtn = CreateFrame("Button", nil, rowFrame)
		removeBtn:SetSize(18, 18)
		removeBtn:SetPoint("LEFT", edit, "RIGHT", 6, 0)
		local rmIcon = removeBtn:CreateTexture(nil, "ARTWORK")
		rmIcon:SetAllPoints()
		rmIcon:SetAtlas("common-search-clearbutton")
		removeBtn:SetNormalTexture(rmIcon)
		local rmHl = removeBtn:CreateTexture(nil, "HIGHLIGHT")
		rmHl:SetAllPoints()
