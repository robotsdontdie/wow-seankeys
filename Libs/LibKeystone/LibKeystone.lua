...
do
	...
	function GetInfo()
		...
		if type(keyLevel) ~= "number" then keyLevel = 0 end
		local keyChallengeMapID = GetOwnedKeystoneChallengeMapID()
		if type(keyChallengeMapID) ~= "number" then keyChallengeMapID = 0 end
...
do
	local throttleTable = { GUILD = 0, PARTY = 0 }
	local timerTable = {}
	local functionTable
	...
	do
		...
		local function SendToParty()
			if timerTable.PARTY then timerTable.PARTY:Cancel(); timerTable.PARTY = nil end
			if IsInGroup() then
		...
		local function SendToGuild()
			if timerTable.GUILD then timerTable.GUILD:Cancel(); timerTable.GUILD = nil end
			if IsInGuild() then
				local keyLevel, keyChallengeMapID, playerRating = GetInfo()
				if keyLevel ~= 0 and LKS.isGuildHidden then keyLevel, keyChallengeMapID = -1, -1 end
		...
		functionTable = { PARTY = SendToParty, GUILD = SendToGuild }
	end
...
do
	local throttleSendTable = { GUILD = 0, PARTY = 0 }
	local statusCheckTable = { GUILD = IsInGuild, PARTY = IsInGroup }
	local timers = {}
	...
	function LKS.Request(channel)
		if not throttleSendTable[channel] then
			...
			for _,func in next, callbackMap do
				securecallfunction(func, keyLevel, keyChallengeMapID, playerRating, pName, channel)
			end
			if statusCheckTable[channel]() then
				local t = GetTime()
				if t - throttleSendTable[channel] > throttleTime then
					if timers[channel] then timers[channel]:Cancel(); timers[channel] = nil end
